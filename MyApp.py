...
# Clean key columns for better UI
...
# Remove any 'Unknown' or empty species for clearer pie chart
...
# Distribution of Pet Species (Pie Chart)
st.subheader("Distribution of Pet Species")
species_count = filtered_df['Species'].value_counts()
fig1, ax1 = plt.subplots()
colors = plt.cm.Paired.colors  # Better color scheme
ax1.pie(species_count, labels=species_count.index, autopct='%1.1f%%', startangle=90, colors=colors)
ax1.axis('equal')  # Equal aspect ratio makes the pie chart round
st.pyplot(fig1)

# Top 10 Primary Breeds (Bar Chart)
